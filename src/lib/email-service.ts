...
const smtpUser = normalizeEnv(process.env.BREVO_USER);
const smtpPass = normalizeEnv(process.env.BREVO_PASS);
const smtpHost = normalizeEnv(process.env.SMTP_HOST) || 'smtp.gmail.com';
const parsedSmtpPort = Number(normalizeEnv(process.env.SMTP_PORT) || 587);
const smtpPort =
  Number.isFinite(parsedSmtpPort) && parsedSmtpPort > 0 ? parsedSmtpPort : 587;

if (!smtpUser || !smtpPass) {
  console.warn(
    '[Email Service] Missing BREVO_USER or BREVO_PASS. Email delivery will fail until credentials are set.',
  );
}
...
<body>
    <div class="container">
        <div class="header">
            <a href="${process.env.NEXT_PUBLIC_APP_URL}" class="logo">MailDetective</a>
        </div>
        <div class="content">
            ${content}
        </div>
        <div class="footer">
            <p>&copy; ${new Date().getFullYear()} MailDetective. All rights reserved.</p>
...
export const sendVerificationEmail = async (email: string, url: string) => {
  const html = getBaseHtml(`
        ...
        <p>Thanks for signing up for MailDetective! Please click the button below to verify your email address.</p>
